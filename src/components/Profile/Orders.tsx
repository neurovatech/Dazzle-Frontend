...
function toOrderModel(o: ApiOrderItem): Order {
  const status: Order["status"] = o.isCancelled
    ? "Cancelled"
    : o.isDelivered
      ? "Delivered"
      : "In Progress";
...
function StatusLabel({ order }: { order: ApiOrderItem }) {
  ...
  if (order.isDelivered)
    return (
      <span className="text-xs font-bold text-green-600 shrink-0">Success</span>
    );
  return (
...
function OrderRow({
  ...
  const items: any = order.comerzOrderItems ?? [];
    ...
    <div className="bg-white dark:bg-[#1c1a17] rounded-2xl border border-gray-100 dark:border-zinc-800 p-3 sm:p-4 flex flex-col gap-3">
      <div className="flex items-start gap-3">
        <div className="w-14 h-14 sm:w-16 sm:h-16 rounded-xl bg-gray-100 dark:bg-zinc-800 flex items-center justify-center shrink-0">
          {thumbnail ? (
            <div className="relative w-14 h-14 sm:w-16 sm:h-16 rounded-xl overflow-hidden bg-gray-100 dark:bg-zinc-800 shrink-0">
              <Image
                src={thumbnail}
                alt={productWithImage?.productName || "Product"}
                fill
                sizes="64px"
