...
const AnnouncementCard: React.FC<{ post: BlogPost }> = ({ post }) => {
  ...
  return (
    <div className="group rounded-xl overflow-hidden border border-gray-100 dark:border-[#1b1b1b] shadow-sm hover:shadow-md transition-shadow bg-white dark:bg-[#1b1b1b] duration-300">
      ...
      <div className="p-4">
        ...
        <h3 className="font-semibold text-sm text-gray-900 dark:text-white mb-2 leading-snug line-clamp-2">
          {post.post_title}
        </h3>
        <p className="text-gray-500 dark:text-gray-300 text-xs mb-4 leading-relaxed line-clamp-2">
          {post.post_caption}
        </p>
        <Link
          href={`/announcement/${post.post_slug}`}
