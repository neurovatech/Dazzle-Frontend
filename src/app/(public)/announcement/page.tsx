...
export const metadata: Metadata = {
  title: "Announcement - Dazzle",
  description: "Announcement the Dazzle team! Explore press openings, career paths, and opportunities in tech, admin, design, and operations at Dazzle Bangladesh.",
};
...
async function getBlogs(page: number, categoryUuid?: string): Promise<BlogsResponse> {
  try {
    ...
    const res = await api.get<unknown>(`/blogs?${qp.toString()}&IsStory=1`, { cache: "no-store" });
...
async function CareerPage({ searchParams }: BlogPageProps) {
  ...
  const buildPageLink = (p: number) => {
    ...
    return `/announcement?${params.toString()}`;
  };
  ...
  return (
    <div className="flex flex-col flex-1 max-w-355 mx-auto lg:px-8 px-4">
      ...
      {/* Posts grid */}
      {blogPosts.length === 0 ? (
        <NoData message="No Announcement found at the moment." />
      ) : (
        <>
