...
export default async function WarrantyPolicy() {
  try {
    const res = await api.get<{ data: Record<string, unknown>[] }>(
      "/pages/membership-policy",
      { cache: "no-store" },
    );
    // console.log("Membership Policy Page - Data fetched successfully:", res);
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    const isExpectedNotFound = /not found/i.test(message);
    if (!isExpectedNotFound) {
      console.error("Error fetching membership policy data:", error);
    }
    ...
    <div className="max-w-355 mx-auto px-4 md:px-12.5 pt-5">
      ...
      <div className="mb-10">
        ...
        <p className="text-sm text-gray-500 dark:text-gray-400 mt-2">
          Dazzle Membership Policy & Terms and Conditions
        </p>
      </div>
